...
data_file <- paste0(year, "_age_", age, "_matched_data_set.csv")
...
method <- "TTE_03"
model_formula <- "Surv(time = time_to_event, event = event_occured) ~ vax_status + start_age + gender + race + dir_cert + prior_infections + (1 | schoolname)"
data_file <- paste0(year, "_age_", age, "_ST_matched_data_set.csv")
...
data_file <- paste0(year, "_age_", age, "_matched_TVC.csv")
...
if (cluster) {
  write.csv(params_list, paste0("/home/amoor53/APSVE_cluster/cleandata/", year, "_age_", age, "_TTE_params.csv"))
} else {
  write.csv(params_list, file = here("cleandata", paste0(year, "_age_", age, "_TTE_params.csv")))
}
