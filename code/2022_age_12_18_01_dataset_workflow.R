...
if (cluster) {
  ...
  Direct_Certification <- read.csv("/home/amoor53/APSVE_cluster/data/2023_directly_certified_school.csv")
    #Schools by Cluster
  ...
  enrollment_newid <- read.csv(here("data", "enrollment_newid_5.6.25.csv"))
    #Direct Certification
  Direct_Certification <- read.csv(here("data", "2023_directly_certified_school.csv"))
    #Schools by Cluster
...
print("")
print("------------------------------")
print("Step 1: Remove NA IDs & Include only 2022-23 School Year")
print("------------------------------")
print("")


start_date <- ymd("2022-08-01")
end_date <- ymd("2023-05-24")
first_sunday <- as.numeric(ymd("2022-08-07")) - 7
...
sample.sizes$school_year_22_23 <- c(ntests, nIDs)
...
length(unique(VScohort.vax$ID)) #18580 students --> 10 missing vaccination status
...
length(unique(VScohort.demographics$ID)) #18590
...
VScohort.demographics$start_age_group <- ifelse(VScohort.demographics$start_age >= 19, 3,
                                                ifelse(VScohort.demographics$start_age >= 12, 2, 
...
d1 <- VScohort.vax[,c("ID","vax_date")] #vaccination dates --> 18580
d2 <- VScohort.demographics[,c("ID", "gender", "race", "start_age", "start_age_group")] #Demographics --> 18590
d3 <- vstests_deid[,c("ID", "schoolname", "result", "resultdate")] #tests --> 633412
...
##### Cleaning Step 4 - Only students aged 12-18 #####

print("")
print("------------------------------")
print("Step 4: Only students aged 12-18")
print("------------------------------")
print("")
...
VScohort.merged <- VScohort.merged[which(VScohort.merged$start_age_group == 2),]

ntests <- nrow(VScohort.merged)
nIDs <- length(unique(VScohort.merged$ID))
sample.sizes$ages_12_18 <- c(ntests, nIDs)
...
problems <- rownames(school.summary[which(school.summary$`n tests` < 10 | school.summary$`n tests` == school.summary$`n vaxed` | school.summary$`n vaxed` == 0 | school.summary$`n + tests` == 0), ])
...
if(cluster) {
  write.csv(VScohort.merged, paste0("/home/amoor53/APSVE_cluster/cleandata/", year, "_age_", age, "_daily_testing_by_ID.csv"))
} else {
  write.csv(VScohort.merged, here("cleandata", paste0(year, "_age_", age, "_daily_testing_by_ID.csv")))
}

# VScohort.merged <- read.csv(here("cleandata", paste0(year, "_age_",  age, "_daily_testing_by_ID.csv")))
...
if (cluster) {
  write.csv(VScohort.alltests, paste0("/home/amoor53/APSVE_cluster/cleandata/", year, "_age_", age, "_positive_tests.csv"))
} else {
  write.csv(here("cleandata", paste0(year, "_age_", age, "_positive_tests.csv")))
}
...
#Save Dataset
if (cluster) {
  write.csv(together, paste0("/home/amoor53/APSVE_cluster/cleandata/", year, "_age_", age, "_weekly_testing_by_ID.csv"))
} else {
  write.csv(together, here("cleandata", paste0(year, "_age_", age, "_weekly_testing_by_ID.csv")))
}
...
# VScohort.tested <- read_csv(here("cleandata", paste0(year, "_age_", age, "_weekly_testing_by_ID.csv"))
...
if (cluster) {
  saveRDS(reg.fit, paste0("/home/amoor53/APSVE_cluster/models/", year, "_age_", age, "_predicted_propensity_scores_model.rds"))
} else {
  saveRDS(reg.fit, here("models", paste0( year, "_age_", age, "_predicted_propensity_scores_model.rds")))
}
...
#Save Dataset
if (cluster) {
  write.csv(VScohort.tested, paste0("/home/amoor53/APSVE_cluster/cleandata/", year, "_age_", age, "_predicted_propensity_scores.csv"))
  write.csv(sample.sizes, paste0("/home/amoor53/APSVE_cluster/results/", year, "_age_", age, "_dataset_workflow_sample_sizes.csv"))
  
} else {
  write.csv(VScohort.tested, here("cleandata", paste0(year, "_age_", age, "_predicted_propensity_scores.csv")))
  write.csv(sample.sizes, here("results", paste0(year, "_age_", age, "_dataset_workflow_sample_sizes.csv")))
}
...
# Save Datasets
if (cluster) {
  write.csv(VScohort.alltested, paste0("/home/amoor53/APSVE_cluster/cleandata/", year, "_age_", age, "_all_tested.csv"))
  write.csv(STcohort.alltested, paste0("/home/amoor53/APSVE_cluster/cleandata/", year, "_age_", age, "_ST_all_tested.csv"))
} else {
  write.csv(VScohort.alltested, file = here("cleandata", paste0(year, "_age_", age, "_all_tested.csv")))
  write.csv(STcohort.alltested, file = here("cleandata", paste0(year, "_age_", age, "_ST_all_tested.csv")))
}
